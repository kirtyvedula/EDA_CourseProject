...
emissionsByYearBalt <- ddply(baltimorePM25, "year", summarise, total = sum(Emissions))

png(filename="Baltimore_Total_Emissions.png")
plot(x    = emissionsByYearBalt$year,
     y    = emissionsByYearBalt$total / 1000,
     type = "l",
     ylab = "Total Emissions from Baltimore (kilotons)",
     xlab = "Year"
)
dev.off()
